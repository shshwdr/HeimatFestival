...
namespace RhythmHeavenMania.Games.Bar
{
    public class Bar : Minigame
    {
        ...
        private void Start()
        {
            allCameraEvents = EventCaller.GetAllInGameManagerList("bar", new string[] { "cameraZoom" });
        ...
        public void Shoot(float beat, bool high, bool far,int type)
        {
            ...
            ball.SetActive(true);
            ball.GetComponent<BarDrink>().startBeat = beat;
            ...
            //if (type == 1)
            {
                ball.GetComponent<BarDrink>().Sprite.sprite = Drinks[type];
            }
